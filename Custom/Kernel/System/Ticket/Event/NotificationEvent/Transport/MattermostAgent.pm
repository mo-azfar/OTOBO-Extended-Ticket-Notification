...
# --
package Kernel::System::Ticket::Event::NotificationEvent::Transport::MattermostAgent;
...
sub SendNotification {
    ...
    my $BotAccessToken = $ConfigObject->Get('MattermostAgent::BotAccessToken');
	my $BotID = $ConfigObject->Get('MattermostAgent::BotID');
	my $BaseURL = $ConfigObject->Get('MattermostAgent::BaseURL');
	
	my $UsernameField = $ConfigObject->Get('MattermostAgent::UsernameField');
	...
	my $TaskName = substr "Mattermost-Recipient".rand().$RecipientUsername, 0, 255;
	...
	my $TaskID = $Kernel::OM->Get('Kernel::System::Scheduler')->TaskAdd(
			...
			Data                     => 
			{
				...
				Function => 'SendMessageMattermostAgent',
				Params   => 
						{
